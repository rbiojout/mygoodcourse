class VisualPreviewInput < SimpleForm::Inputs::FileInput
  def input(wrapper_options = nil)
    ...
    if object.send("#{attribute_name}?")
      ...
      out << "<div>".html_safe
      out << template.image_tag(object.send(attribute_name).tap {|o| break o.send(version) if version}.send('url'), class: 'img-responsive')
      out << "</div>".html_safe
    end
