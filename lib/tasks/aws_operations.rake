...
namespace :aws do
  ...
  task copy_bucket_locally: :environment do
    ...
    directory.files.each do |s3_file|
      ...
      next if File.file?(upload_folder + s3_file.key)
      # create the folders
  ...
  task clean_attachments: :environment do
    ...
    directory.files.each do |s3_file|
      # retrieve date
      next unless s3_file.last_modified < Time.now - 6.hours
      next unless File.dirname(s3_file.key).include?'uploads/attachment/file'
      counter += 1
