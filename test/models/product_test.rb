...
class ProductTest < ActiveSupport::TestCase
  ...
  test 'count filtered by cycles' do
    ...
    assert_equal 2, Product.count_active_filtered_for_cycle(cycles(:C1),
                                                            families(:F1), nil)
    assert_equal 1, Product.count_active_filtered_for_cycle(cycles(:C1),
                                                            families(:F1), categories(:F1C1))
    assert_equal 0, Product.count_active_filtered_for_cycle(cycles(:C1),
                                                            families(:F2), nil)
    assert_equal 0, Product.count_active_filtered_for_cycle(cycles(:C1),
                                                            families(:F1), categories(:F2C1))
    assert_equal 0, Product.count_active_filtered_for_cycle(cycles(:C1),
                                                            nil, categories(:F2C1))
  end
  ...
  test 'count filtered by levels' do
    ...
    assert_equal 1, Product.count_active_filtered_for_level(levels(:C1L1),
                                                            families(:F1), nil)
    assert_equal 1, Product.count_active_filtered_for_level(levels(:C1L1),
                                                            families(:F1), categories(:F1C1))
    assert_equal 0, Product.count_active_filtered_for_level(levels(:C1L1),
                                                            families(:F2), nil)
    assert_equal 0, Product.count_active_filtered_for_level(levels(:C1L1),
                                                            families(:F1), categories(:F2C1))
    assert_equal 0, Product.count_active_filtered_for_level(levels(:C1L1),
  ...
  test 'count filtered by families' do
    ...
    assert_equal 2, Product.count_active_filtered_for_family(families(:F1),
                                                             cycles(:C1), nil)
    assert_equal 1, Product.count_active_filtered_for_family(families(:F1),
                                                             cycles(:C1), levels(:C1L1))
    assert_equal 0, Product.count_active_filtered_for_family(families(:F1),
                                                             cycles(:C2), nil)
    assert_equal 0, Product.count_active_filtered_for_family(families(:F1),
                                                             cycles(:C1), levels(:C2L1))
    assert_equal 0, Product.count_active_filtered_for_family(families(:F1),
                                                             nil, levels(:C2L1))
  end
  ...
  test 'count filtered by categories' do
    ...
    assert_equal 1, Product.count_active_filtered_for_category(categories(:F1C1),
                                                               cycles(:C1), nil)
    assert_equal 1, Product.count_active_filtered_for_category(categories(:F1C1),
                                                               cycles(:C1), levels(:C1L1))
    assert_equal 0, Product.count_active_filtered_for_category(categories(:F1C1),
                                                               cycles(:C2), nil)
    assert_equal 0, Product.count_active_filtered_for_category(categories(:F1C1),
                                                               cycles(:C1), levels(:C2L1))
    assert_equal 0, Product.count_active_filtered_for_category(categories(:F1C1),
